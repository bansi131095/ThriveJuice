...
class ShopList_VC: UIViewController {

    
    @IBOutlet weak var lbl_title: UILabel!
    @IBOutlet weak var vw_buy: CardView!
    @IBOutlet weak var collect_vw: UICollectionView!
    @IBOutlet weak var lbl_totalProduct: UILabel!
    @IBOutlet weak var lbl_TotalPrice: UILabel!
    @IBOutlet weak var lbl_sortProduct: UILabel!
    @IBOutlet weak var lbl_sortFilterData: UILabel!
    @IBOutlet weak var lbl_cartTotal: UILabel!
    @IBOutlet weak var vw_EmptyData: UIView!
    @IBOutlet weak var lbl_emptyData: UILabel!
...
//    var Totalcart = 0
    ...
    override func viewDidLoad() {
        ...
        self.lbl_title.text = categoryName
        UserDefaults.standard.set(categoryName, forKey: "selectCategory")
        self.lbl_sortProduct.text = categoryName
        UserDefaults.standard.set("Popularity", forKey: "selectPrice")
        self.lbl_sortFilterData.text = "Popularity"
        // Do any additional setup after loading the view.
    }
    
    override func viewDidAppear(_ animated: Bool) {
        super.viewDidAppear(animated)
        UserDefaults.standard.set(self.categoryId, forKey: "selectCategory")
    ...
    func updateDataFromSecondScreen(data: String) {
    ...
    @IBAction func act_AllProduct(_ sender: UIButton) {
    ...
    @IBAction func act_PriceFilter(_ sender: UIButton) {
        let filter = self.storyboard?.instantiateViewController(withIdentifier: "Filter_VC") as! Filter_VC
        filter.modalPresentationStyle = .overFullScreen
        filter.isCategory = false
        filter.delegate = self
        self.present(filter, animated: true)
    }
        ...
        WebService.call.POSTT(filePath: global.shared.URL_Products, params: paramer, enableInteraction: false, showLoader: true, viewObj: self, onSuccess: { [self] (result, success) in
            ...
            if let eventResponseModel:ProductModel = Mapper<ProductModel>().map(JSONObject: result) {
                if let arr = eventResponseModel.products, arr.count != 0 {
                    ...
                    self.arr_FilterProduct = self.arr_Product
                    ...
                    if offset == 0 {
                        ...
                        self.lbl_emptyData.text = categoryName + " doesn't have products in " + ordertype
                    } else {
                        self.collect_vw.isHidden = false
                        self.vw_EmptyData.isHidden = true
                    }
...
extension ShopList_VC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension ShopList_VC: FilterDataDelegate {
    ...
    func didFinishTask(data: String, type: String, name: String) {
        if type == "Category" {
            ...
            self.lbl_sortProduct.text = name
            self.lbl_title.text = name
            self.call_ProductAPI()
        } else if type == "Price" {
            offset = 0
            self.str_offset = "\(offset)"
            self.lbl_sortFilterData.text = data
            if data == "Popularity" {
                str_filter = ""
//                self.arr_Product = self.arr_FilterProduct
//                self.collect_vw.reloadData()
            } else if data == "Price: Low to High" {
                str_filter = "Product_Price_L_H"
//                let itemsWithParsedPrice = self.arr_FilterProduct.map { item -> (Products, Double) in
//                    let val = item.product_Size?[0].product_Price ?? "0"
//                    guard let priceValue = Double(val) else {
//                        // Handle invalid price strings
//                        return (item, 0.0)
//                    }
//                    return (item, priceValue)
//                }
//                let arr = itemsWithParsedPrice.sorted { $0.1 < $1.1 }
//                // Extract the final sorted and filtered items
//                let resultItems = arr.map { $0.0 }
//                self.arr_Product = resultItems
//                self.collect_vw.reloadData()
            } else if data == "Price: High to Low" {
                str_filter = "Product_Price_H_L"
//                let itemsWithParsedPrice = self.arr_FilterProduct.map { item -> (Products, Double) in
//                    let val = item.product_Size?[0].product_Price ?? "0"
//                    guard let priceValue = Double(val) else {
//                        // Handle invalid price strings
...
//                self.collect_vw.reloadData()
                ...
                str_filter = "Name_A_Z"
//                let sortedItems = self.arr_FilterProduct.sorted { $0.product_Name ?? "" < $1.product_Name ?? "" }
//                self.arr_Product = sortedItems
//                self.collect_vw.reloadData()
